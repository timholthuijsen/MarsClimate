...
! + possibility to concatenate concat files with a coherent 
!   time axis in the output, and to redistribute the time axis
!   from an offset given by the user. AB, April 2020
! ********************************************************
...
integer :: memolatlen=0,memolonlen=0,memoaltlen=0,memoctllen=0
...
! memoctllen: # of elements of ctl(), read from the first input file
...
!==============================================================================
write(*,*) 
write(*,*) "-- Program written specifically for NetCDF 'diagfi'/'concat' files"
write(*,*) "                     from the martian GCM --"
write(*,*) 
...
!==============================================================================
! 1.2. Ask for starting day value (starttimeoffset)
!==============================================================================
...
!==============================================================================
! 2.2.1 Check presence and read "controle" dimension & variable from input file
!==============================================================================
   ierr=NF_INQ_DIMID(nid,"index",ctldim)
   if (ierr.NE.NF_NOERR) then
      write(*,*) 'Dimension <index> is missing in file '//file(i)
      write(*,*) "The program continues..."
      ctllen=0
      !stop ""
   else
      ierr=NF_INQ_VARID(nid,"controle",ctlvar)
      if (ierr.NE.NF_NOERR) then
         write(*,*) 'Field <controle> is missing in file '//file(i)
         write(*,*) "The program continues..."
         ctllen=0
         !stop ""
      else
...
!==============================================================================
! 2.3 Read "Time" dimension & variable from input file
!==============================================================================
...
!        artificially estimating "previous_last_output_time" for first file read: 
   ...
   ! and that either all the files or no file contain the controle variable
      if (memolatlen/=latlen) then
           ...
           stop ""
       else if (memoctllen/=ctllen) then
           write(*,*) "ERROR: Not the same controle axis"
           stop ""
       endif
   endif ! of if (i==1)
...
!==============================================================================
! 2.4 Write/extend "Time" dimension/values in output file
!==============================================================================
...
! In case there is a "valid_range" attribute
   ...
   ! Deallocate controle if needed
   if (ctllen.ne.0) deallocate(ctl)
...
write(*,*);write(*,*) "Program concatnc completed !"
contains
...
if (axis.eq."ls") then 
   ...
   call def_var(nout,"Ls","Ls (Solar Longitude)","degrees",1, (/timedimout/),nvarid,ierr)
end if
