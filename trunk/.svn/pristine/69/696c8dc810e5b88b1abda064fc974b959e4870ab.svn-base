...
IF(.NOT.file_ok) THEN
  ...
  CALL abort_physic("suaer","missing file "//trim(file_id(iaer,idomain)),1)
ENDIF
OPEN(UNIT=file_unit,&
  FILE=TRIM(datadir)//&
  '/'//TRIM(file_id(iaer,idomain)),&
...
DO WHILE (.NOT.endwhile)
  ...
  if (read_ok.ne.0) then
    ...
    TRIM(datadir)//&
    '/'//TRIM(file_id(iaer,idomain))
    call abort_physic("suaer","problem reading "//trim(file_id(iaer,idomain)),1)
  endif
  IF ((scanline(1:1) .ne. '#').and.&
    ...
    CASE(1) reading1_seq ! nwvl ----------------------------
        ...
        if (read_ok.ne.0) then
          ...
          TRIM(datadir)//&
          '/'//TRIM(file_id(iaer,idomain))
          call abort_physic("suaer",&
               "problem reading "//trim(file_id(iaer,idomain)),1)
        endif
        jfile = jfile+1
    ...
    CASE DEFAULT reading1_seq ! ---------------------------- 
        ...
        call abort_physic("suaer",&
               "problem loading optical properties",1) 
...
! test that there is enough room to store the data
 if (isize.gt.nsizemax) then
   ...
   call abort_physic("suaer","nsizemax too small",1)
 endif
! ------------------------------------------------
