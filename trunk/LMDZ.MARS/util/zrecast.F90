...
! FF 05/2021 : Force interpolation in log space for variable starting by "num_"
! FF 05/2021 : "phisinit" added to the putput file
! FF 05/2021 : Conversion to pressure coordinate possible even if temperature
!              not available
!
implicit none
...
integer lon_varid,lat_varid,alt_varid,time_varid,ctl_varid,phis_varid
...
integer,dimension(2) :: phisdatashape ! shape of 2D (surface) datasets
...
logical :: have_temp ! Flag: true if temperature is available in some way
...
if (ierr.ne.NF_NOERR) then
  ...
  if (ierr.ne.NF_NOERR) then
     ...
     if (ierr.ne.NF_NOERR) then
       write(*,*) "Error: Failed to get temp, t or teta ID"
       have_temp=.false.
       if (ztype.ne.1) stop
     endif
       ierr=NF_GET_VAR_REAL(infid,tmpvarid,teta)
       if (ierr.ne.NF_NOERR) then
         write(*,*) "Failed reading atmospheric temperature"
         if (ztype.ne.1) stop
       endif
...
! Add phisinit in outputfile
phisdatashape(1)=lon_dimid
phisdatashape(2)=lat_dimid
ierr=NF_DEF_VAR(outfid,"phisinit",NF_REAL,2,phisdatashape,phis_varid)
...
if (ierr.ne.NF_NOERR) then
  stop "Error: Problem writing units for geopotential phisinit"
endif
...
if ((ztype.eq.1).and.(have_temp)) then ! pressure vertical coordinate
  ! zareoid dataset
...
! write phisinit
  ...
  if (ierr.ne.NF_NOERR) then
    write(*,*) "Error: Could not write phisinit to output file"
    write(*,*) NF_STRERROR(ierr)
    stop
  endif
...
if (ztype.eq.2) then
  ...
  if ((var(i)(1:3).eq.'rho').or.(var(i)(1:4).eq.'num_')) then
    j=1
