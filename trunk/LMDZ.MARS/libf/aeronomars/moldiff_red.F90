...
!      real hco2(ncompdiff),ho
      ...
      real*8 PhiauxH(ngrid),PhiauxH2(ngrid),PhiauxD(ngrid)
      real*8 hp(nlayer)
      real*8 pp(nlayer)
      real*8 pint(nlayer)
...
!	if (i_h .ne. 1000) PhiEscH=PhiEscH+wi(i_h)*Nrafk(nlraf,i_h)*cell_area(ig) ! in s-1
        if (i_h .ne. 1000) PhiauxH(ig)=wi(i_h)*Nrafk(nlraf,i_h)*cell_area(ig) ! in s-1
!	if (i_h2 .ne. 1000) PhiEscH2=PhiEscH2+wi(i_h2)*Nrafk(nlraf,i_h2)*cell_area(ig) ! in s-1 (U in m/s, aire in m2, Nrafk in m-3)
        if (i_h2 .ne. 1000) PhiauxH2(ig)=wi(i_h2)*Nrafk(nlraf,i_h2)*cell_area(ig)
!        if (i_d .ne. 1000) PhiEscD=PhiEscD+wi(i_d)*Nrafk(nlraf,i_d)*cell_area(ig)
        if (i_d .ne. 1000) PhiauxD(ig)=wi(i_d)*Nrafk(nlraf,i_d)*cell_area(ig)
...
!	Rrafk(l,iq)=Rraf(l)*Qraf(l,iq)
	...
	enddo
        !Temporary: eliminate modification to include Wilke's formulation
        !back to the old scheme to check effect 
	!D(l)=1d0/interm
