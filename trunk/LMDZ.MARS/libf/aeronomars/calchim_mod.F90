...
!            call abort_physic("calchim","missing hdo_vap tracer",1)
         ...
         i_h3oplus=igcm_h3oplus
         if(ionchem) then
            if (i_h3oplus == 0) then
               write(*,*) "calchim: Error, no H3O+ tracer !!!"
               write(*,*) "H3O+ is needed if O2+ is in traceur.def"
               call abort_physic("calchim","missing h3oplus tracer",1)
            else
               nbq = nbq + 1
               niq(nbq) = i_h3oplus
            end if
            ...
            if (i_h3oplus /= 0) then
               write(*,*) "calchim: Error: H3O+ is present, but O2+ is not!!!"
               write(*,*) "Both must be in traceur.def if ion chemistry wanted"
            endif
         endif
         i_ohplus=igcm_ohplus
         if(ionchem) then
            if (i_ohplus == 0) then
               write(*,*) "calchim: Error, no OH+ tracer !!!"
               write(*,*) "OH+ is needed if O2+ is in traceur.def"
               call abort_physic("calchim","missing ohplus tracer",1)
            else
               nbq = nbq + 1
               niq(nbq) = i_ohplus
            end if
            ...
            if (i_ohplus /= 0) then
               write(*,*) "calchim: Error: OH+ is present, but O2+ is not!!!"
               write(*,*) "Both must be in traceur.def if ion chemistry wanted"
            endif
         endif
         i_elec = igcm_elec
...
!        call main photochemistry routine
            ...
            call photochemistry(nlayer,nq,nbq,ionchem,deutchem,           &
                           ...
                           jo3,jh2o,em_no,em_o2,taucol,iter)
