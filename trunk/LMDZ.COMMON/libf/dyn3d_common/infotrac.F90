...
! Local variables
    ...
    INTEGER :: iq, new_iq, iiq, jq, ierr, ierr1, ierr2, ierr3, ierr4 !MVals: add ierr1 et ierr4 for the isotope case
    INTEGER :: ifils,ipere,generation ! CRisi
    LOGICAL :: continu,nouveau_traceurdef
    LOGICAL :: moderntracdef=.false. ! JVO, YJ : For planets modern traceur.def
...
!! - Modif. by JVO and YJ for modern planetary traceur.def ---------------
          ...
          IF (trim(line).ne.'#ModernTrac-v1') THEN ! Test modern traceur.def
             READ(line,*) nqtrue ! Try standard traceur.def
          ELSE
             moderntracdef = .true.
             DO
               READ(90,'(A)',iostat=ierr1) line
               IF (ierr1==0) THEN
                 IF (index(line,'#').ne.1) THEN ! Allows arbitary number of comments lines in the header
                   READ(line,*) nbtr
                   WRITE(lunout,*) trim(modname),': Number of tracers in the physics: nbtr=',nbtr
                   EXIT
                   ! JVO 20 : Here nbtr corresponds to ALL what we consider as modern 'tracers' (species)
                   ! from physics, including non-dynamic species (radiative, compo..), and nqtrue will be set
                   ! below, according to the flags in traceur.def, by read and rewind.
                 ENDIF
               ELSE ! If pb, or if reached EOF without having found nbtr
                 CALL abort_gcm('infotrac_init','Unable to read numbers of tracers in traceur.def',1)
               ENDIF
             ENDDO
...
!
    IF (.NOT.moderntracdef) ALLOCATE(tnom_0(nqtrue), hadv(nqtrue), vadv(nqtrue),tnom_transp(nqtrue))
!
!jyg<
...
!>jyg
    ...
    ELSE  ! not Earth
       ...
       IF (.not.moderntracdef) THEN ! Standard traceur.def or no traceur.def file
         ...
         IF(ierr.EQ.0) THEN
            ...
            DO iq=1,nqtrue
              ...
              ! if format is hadv, vadv, tnom_0, tnom_transp
              read(line,*,iostat=ierr1) hadv(iq),vadv(iq),tnom_0(iq),tnom_transp(iq)
              if (ierr1.ne.0) then
              ! assume format is hadv,vadv,tnom_0
               ...
               if (ierr2.ne.0) then
                ...
                if (ierr3.ne.0) then
                  !assuming values of hadv et vadv
                  hadv(iq)=10
                  vadv(iq)=10
                  read(line,*, iostat=ierr4) tnom_0(iq), tnom_transp(iq)
                  if (ierr4.ne.0) then
                  ! assume only tnom0 is provided (hadv and vadv default to 10)
                    read(line,*) tnom_0(iq)
                    tnom_transp(iq)='air' 
                  endif
                else
                  !format is tnom0,hadv,vadv
                  tnom_transp(iq)='air' ! no isotopes
                endif ! of if (ierr3.ne.0)
               else
                 ! format is hadv,vadv,tnom_0
                 tnom_transp(iq)='air' ! no isotopes
               endif ! of if (ierr2.ne.0)
              endif ! of if(ierr1.ne.0)
       ...
       ELSE ! Modern traceur.def (moderntracdef=true) - Authors : JVO, YJ - 2020

         nqtrue = 0
         DO iq=1,nbtr ! This loops on ALL the 'tracers' including the non-dyn ones.
           READ(90,'(A)') line ! store the line from traceur.def
           IF (index(line,'is_dyn=0').ne.0) CYCLE ! Only if explicitly not dyn., otherwise default behaviour
           nqtrue = nqtrue + 1
         ENDDO
         WRITE(lunout,*) trim(modname),': Number of tracers in the dynamics: nqtrue=',nqtrue
         ALLOCATE(tnom_0(nqtrue), hadv(nqtrue), vadv(nqtrue),tnom_transp(nqtrue))

         ! Now that we know the dyn. tracers and allocated variables, let's read again the file.
         REWIND(90)
         DO
           READ(90,'(A)') line ! no need to check on iostat, already done at first read
           IF (index(line,'#').ne.1) THEN ! Skip arbitary number of commented lines in the header
             READ(line,*) ! This is the line of nbtr (see above)
             EXIT
           ENDIF
         ENDDO
         iiq=0
         DO iq=1,nbtr ! This loops on ALL the 'tracers' including the non-dyn ones.
           ...
           READ(90,'(A)') line ! store the line from traceur.def
           IF (index(line,'is_dyn=0').ne.0) CYCLE ! Skip the non-dyn ones
           iiq = iiq +  1 ! This ensure that iiq is on 1:nqtrue and not 1:nbtr
           ! Name must be first parameter in this version, but all others can be in any order
           read(line,*) tnom_0(iiq)
           write(*,*)"infotrac_init: iq=",iiq,"noms(iq)=",trim(tnom_0(iiq))

           if (index(line,'vadv=') /= 0) then
             read(line(index(line,'vadv='   )+len('vadv='):),*) vadv(iiq)
             write(*,*) ' Parameter value (traceur.def) : vadv=', vadv(iiq)
           else
             vadv(iiq)=10
             write(*,*) ' Parameter value (default)     : vadv=',vadv(iiq)
           end if

           if (index(line,'hadv=') /= 0) then
             read(line(index(line,'hadv='   )+len('hadv='):),*) hadv(iiq)
             write(*,*) ' Parameter value (traceur.def) : hadv=', hadv(iiq)
           else
             hadv(iiq)=10
             write(*,*) ' Parameter value (default)     : hadv=',hadv(iiq)
           end if

           if (index(line,'tnom_transp=' ) /= 0) then
             read(line(index(line,'tnom_transp='   )+len('tnom_transp='):),*) tnom_transp(iiq)
             write(*,*) ' Parameter value (traceur.def) : tnom_transp=', tnom_transp(iiq)
           else
             tnom_transp(iiq)='air'
             write(*,*) ' Parameter value (default)     : tnom_transp=',tnom_transp(iiq)
           end if

         ENDDO
         CLOSE(90)

       ENDIF ! if (moderntracdef)
