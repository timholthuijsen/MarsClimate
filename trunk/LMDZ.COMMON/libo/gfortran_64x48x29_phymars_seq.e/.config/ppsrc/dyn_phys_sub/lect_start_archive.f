...
! -----------------
      ...
      REAL,INTENT(OUT) :: watercap(ngrid) ! infinite polar cap
...
!      allocate(tsoilold(imold+1,jmold+1,nsoilmx))
      ...
      allocate(watercapold(imold+1,jmold+1))
...
c-----------------------------------------------------------------------
c 5.1 Lecture des champs 2D (co2ice, emis,ps,tsurf,Tg[10], q2surf, tauscaling,totcloudfrac,watercap)
c-----------------------------------------------------------------------
...
c
     ...
     &                            totcloudfracold)
      ...
      ENDIF
c
      ierr = NF_INQ_VARID (nid, "watercap", nvarid)
      IF (ierr .NE. NF_NOERR) THEN
         PRINT*, "lect_start_archive: <watercap> not in file"
         watercapold(:,:) = 0.
      ELSE
        ierr = NF_GET_VARA_DOUBLE(nid, nvarid,start,count,
     &                            watercapold)
...
c Watercap
      call interp_horiz (watercapold,watercaps,imold,jmold,iim,
     &                   jjm,1,rlonuold,rlatvold,rlonu,rlatv)
      call gr_dyn_fi (1,iim+1,jjm+1,ngrid,watercaps,watercap)
