     ...
     &                      igcm_h2o_vap ! index of water vapor tracer
...
!     ---------
      ...
      REAL,intent(out) :: pdhadj(ngrid,nlay)
      REAL,intent(in) :: pplay(ngrid,nlay),pplev(ngrid,nlay+1)
      REAL,intent(in) :: ppopsk(ngrid,nlay)
      REAL,intent(in) :: pu(ngrid,nlay),pdufi(ngrid,nlay)
      REAL,intent(out) :: pduadj(ngrid,nlay)
      REAL,intent(in) :: pv(ngrid,nlay),pdvfi(ngrid,nlay)
...
!     check conservation
     ...
     &           masse*(zq2(i,l,iq)-zq(i,l,iq))/ptimestep 
         ...
         do l = 1, nlay
            ...
            call abort_physic("convadj","crashed",1)
         endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      ENDDO ! of DO jj = 1, jcnt   ! loop on every convective grid point
...
!         ig=1
!         iq =1 
!         write(*,*)'**convadj: l, pq(ig,l,iq),zq(ig,l,iq),zq2(ig,l,iq)'  
!         do l=nlay,1,-1
