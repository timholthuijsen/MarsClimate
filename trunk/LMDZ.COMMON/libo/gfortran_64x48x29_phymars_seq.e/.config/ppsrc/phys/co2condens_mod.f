...
c MVals: variables isotopes
      REAL Ratio1(nlayer),Ratiom1(nlayer+1)
...
c           Van Leer scheme:
            ...
            ! MVals: loop over the fathers ("peres")
            do iq=1,nqperes
             do l=1,nlayer
              zq1(l)=zqc(l,iq)
             enddo
             ...
             if (nqfils(iq).gt.0) then
              ...
              do l=1,nlayer
               if (zqc(l,iq).gt.qperemin) then
                 Ratio1(l)=zqc(l,iq2)/zqc(l,iq)
               else
                 Ratio1(l)=0.
               endif
               masseq(l)=max(masse(l)*zqc(l,iq),masseqmin)
               wq(l)=w(l)*zqm(l,iq)
              enddo
              Ratiom1(1)=zqm(1,iq2)
              call vl1d(nlayer,Ratio1,2.,masseq,wq,Ratiom1)
              zqm(1,iq2)=Ratiom1(1)*zqc(1,iq)
              do l=2,nlayer
               zqm(l,iq2)=Ratiom1(l)*zqm(l,iq)
              enddo
...
c   --------------------------------------------------------------------
     ...
     &                      rho_dust, nuice_sed, nuice_ref,r3n_q
      ...
      use dust_param_mod, only: freedust
      IMPLICIT NONE
      include "callkeys.h" ! for the flags water and microphys
c
c
c   Arguments:
      INTEGER,INTENT(IN) :: ngrid  ! number of atmospheric columns
