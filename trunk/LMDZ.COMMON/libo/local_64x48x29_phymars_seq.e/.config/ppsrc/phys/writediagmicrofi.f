      ...
      subroutine writediagmicrofi(ngrid,imicro,microstep,microtimestep,
     &                            nom,titre,unite,dim,px)
...
!         Nov 2020 Margaux: creates an ouput file for microphysics (diagmicrofi.nc), 
!                           the temporal dimension "microtime" has been added
!                           to be able to get the outputs for each sub-timestep, 
!                           dimensions are: (time,microtime,alt,lat,lon)
!
!  parametres (input) :
!  ----------
...
! Arguments on input:
      ...
      integer,intent(in) :: imicro ! time subsampling for coupled water microphysics & sedimentation
      integer,intent(in) :: microstep ! time subsampling step variable
      real,intent(in) :: microtimestep  ! integration timestep for coupled water microphysics
...
!$OMP THREADPRIVATE(ntime)
      integer :: idim,idim_micro,varid
      integer :: nid
      character(len=*),parameter :: fichnom="diagmicrofi.nc"
      integer, dimension(5) :: id
      integer, dimension(5) :: edges,corner

! Added to use diagfi.def to select output variable
      logical,save :: diagmicrofi_def
      logical :: getout
...
!$OMP THREADPRIVATE(firstcall) 	!diagmicrofi_def,n_nom_def,nom_def read in diagfi.def
...
!$OMP MASTER
  !      Open diagmicrofi.def definition file if there is one:
         open(99,file="diagmicrofi.def",status='old',form='formatted',
     s   iostat=ierr2)

         if(ierr2.eq.0) then
            diagmicrofi_def=.true.
            write(*,*) "******************"
            write(*,*) "Reading diagmicrofi.def"
            write(*,*) "******************"
            do n=1,n_nom_def_max
              read(99,fmt='(a)',end=88) nom_def(n)
              write(*,*) 'Output in diagmicrofi: ', trim(nom_def(n))
            end do 
 88         continue
            if (n.ge.n_nom_def_max) then
               write(*,*)"n_nom_def_max too small in ", 
     &                    "microwritediagmicrofi.F:",n
               call abort_physic("writediagmicrofi",
     &             "n_nom_def_max too small",1)
            end if 
            n_nom_def=n-1
            close(99)
         else
            diagmicrofi_def=.false.
         endif
!$OMP END MASTER
!$OMP BARRIER
      END IF ! of IF (firstcall)

! Get out of write_diagmicrofi if there is diagfi.def AND variable not listed
!  ---------------------------------------------------------------------
      if (diagmicrofi_def) then
          getout=.true.
          do n=1,n_nom_def
...
! Initialisation of 'firstnom' and create/open the "diagmicrofi.nc" NetCDF file
! ------------------------------------------------------------------------
! (at very first call to the subroutine, in accordance with diagmicrofi.def)
...
!#endif
     ...
     .          "days since 0000-00-0 00:00:00")
         ! Define the 'microtime' dimension
         ierr = nf_def_dim(nid,"microtime",imicro,idim_micro)
         ierr = NF_DEF_VAR (nid, "microtime", NF_FLOAT, 1, 
     .                                idim_micro,varid)
         ! Add a long_name attribute
         ierr = NF_PUT_ATT_TEXT (nid, varid, "long_name",
     .          9,"microtime")
         ! Add a units attribute
         ierr = NF_PUT_ATT_TEXT (nid,varid,'units',5,"steps")
...
!#endif
           if (ierr.ne.NF_NOERR) then
              write(*,*) "***** PUT_VAR matter in writediagmicrofi_nc"
              write(*,*) "***** with time"
              write(*,*) 'ierr=', ierr,": ",NF_STRERROR(ierr)  
c             call abort
           endif

           write(6,*)'WRITEDIAGMICROFI: date= ', date, zitau
        ...
        if (nom.eq.firstnom) then
           ! compute corresponding date (in days and fractions thereof)
           subdate=subdate+microtimestep

           ! Get NetCDF ID of 'microtime' variable
           ierr= NF_INQ_VARID(nid,"microtime",varid)
!#else
           ierr= NF_PUT_VARA_REAL(nid,varid,microstep,1,subdate)
!#endif
           if (ierr.ne.NF_NOERR) then
              write(*,*) "***** PUT_VAR matter in writediagmicrofi_nc"
              write(*,*) "***** with time"
...
c             call abort
           ...
           write(6,*)'WRITEDIAGMICROFI: subdate= ', subdate, zitau
        end if ! of if (nom.eq.firstnom)
...
!#endif
           ...
           if (ierr.ne.NF_NOERR) then
              write(*,*) "***** PUT_VAR problem in writediagmicrofi"
              write(*,*) "***** with dx3: ",trim(nom)
              write(*,*) 'ierr=', ierr,": ",NF_STRERROR(ierr)
              call abort_physic("writediagmicrofi",
     &             "failed writing "//trim(nom),1)
           endif 
...
! corresponding dimensions
              ierr= NF_INQ_DIMID(nid,"longitude",id(1))
              ierr= NF_INQ_DIMID(nid,"latitude",id(2))
              ierr= NF_INQ_DIMID(nid,"Microtime",id(3))
...
! Create the variable if it doesn't exist yet
             ...
             if (ntime==0) then
              write(*,*) "DIAGFI Error: failed creating variable ",
     &                   trim(nom)
              write(*,*) "it seems it already exists!"
              call abort_physic("writediagmicrofi",
     &             trim(nom)//" already exists",1)
...
!#endif     
           ...
           if (ierr.ne.NF_NOERR) then
              write(*,*) "***** PUT_VAR matter in writediagmicrofi"
...
!---------------------------------------------------
         ...
         if (is_parallel) then
           write(*,*) "writediagmicrofi error: dim=1 not implemented ",
     &                 "in parallel mode. Problem for ",trim(nom)
              call abort_physic("writediagmicrofi",
     &             "failed writing "//trim(nom),1)
         endif
!         Passage variable physique -->  physique dynamique
...
! corresponding dimensions
              ...
              ierr= NF_INQ_DIMID(nid,"microtime",id(2))
...
!---------------------------------------------------
          ...
          if (is_master) then
           ...
           if (ierr /= NF_NOERR) then
! corresponding dimensions
              ierr= NF_INQ_DIMID(nid,"Microtime",id(1))
